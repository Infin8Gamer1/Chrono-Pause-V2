...
namespace Behaviors
{
	...
	void MouseClickCollisionHandler(GameObject& mouseClick, GameObject& other)
	{
		...
		MouseClick* click = static_cast<MouseClick*>(mouseClick.GetComponent("MouseClick"));
		...
		if (Input::GetInstance().IsKeyDown(VK_LBUTTON) && !click->lastClickDown)
		{
			...
			UIButton* button = static_cast<UIButton*>(other.GetComponent("UIButton"));
			if (button)
			{
				button->Call();
				return;
			...
			if (click->selectedObject != click->freezableObject)
			{
				...
				click->freezableSprite = static_cast<Sprite*>(click->freezableObject->GetComponent("Sprite"));
